...
o.spec("DesktopIntegrator Test", () => {
	...
	o("Darwin enable when off", async function () {
		n.setPlatform('darwin')
		const {electronMock, fsExtraMock, cpMock} = standardMocks()
		const integrator = new DesktopIntegrator(electronMock, fsExtraMock, cpMock)
	...
	o("Darwin disable when off", async function () {
		n.setPlatform('darwin')
		const {electronMock, fsExtraMock, cpMock} = standardMocks()
		const integrator = new DesktopIntegrator(electronMock, fsExtraMock, cpMock)
	...
	o("Darwin enable when on", async function () {
		n.setPlatform('darwin')
		...
		const {fsExtraMock, cpMock} = standardMocks()
		const integrator = new DesktopIntegrator(electronMock, fsExtraMock, cpMock)
		await integrator.enableAutoLaunch()
	...
	o("Darwin disable when on", async function () {
		n.setPlatform('darwin')
		const electronMock = n.mock('electron', electron).with({
			app: {
				getLoginItemSettings() {return {openAtLogin: true}}
	...
	o("Linux enable when off", async function () {
		setupLinuxEnv()
		const {electronMock, fsExtraMock, cpMock} = standardMocks()
		const integrator = new DesktopIntegrator(electronMock, fsExtraMock, cpMock)
		...
		o(fsExtraMock.mkdirSync.args.length).equals(1)
		o(fsExtraMock.mkdirSync.args[0]).equals("/app/path/file/.config/autostart")
		resetLinuxEnv()
	})

	o("Linux disable when off", async function () {
		setupLinuxEnv()
		const {electronMock, fsExtraMock, cpMock} = standardMocks()
		const integrator = new DesktopIntegrator(electronMock, fsExtraMock, cpMock)
	...
	o("Linux enable when on", async function () {
		setupLinuxEnv()
	...
	o("Linux disable when on", async function () {
		setupLinuxEnv()
		standardMocks()
		const fsExtraMock = n.mock('fs-extra', fsExtra).with({
	...
	o("Win32 enable when off", async function () {
		n.setPlatform('win32')
	...
	o("Win32 disable when off", async function () {
		n.setPlatform('win32')
		const {electronMock, fsExtraMock, cpMock, winregMock} = standardMocks()
		const integrator = new DesktopIntegrator(electronMock, fsExtraMock, cpMock)
	...
	o("Win32 enable when on", async function () {
		n.setPlatform('win32')
	...
	o("Win32 disable when on", async function () {
		n.setPlatform('win32')
		itemToReturn = "not undefined"
		const {electronMock, fsExtraMock, cpMock, winregMock} = standardMocks()
		const integrator = new DesktopIntegrator(electronMock, fsExtraMock, cpMock)
